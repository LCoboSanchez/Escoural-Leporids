...
# Load data
data <- read.csv("7_HCPC_Kmeans_tapho_rr.csv", sep = ";", na.strings = "na")
...
ggsave("Figure10_rr.png", plot = factor_map, width = 10, height = 8, dpi = 300)
...
factor_map <- fviz_cluster(kmeans_result, data=data_imputed, geom = "point", ellipse.type = "convex", palette = "Set2", ggtheme = theme_minimal(),shape=19)
...
plot(factor_map)
# Save the plot with 300 dpi resolution
ggsave("Figure11_rr.png", plot = factor_map, width = 10, height = 8, dpi = 300)
...
write.table(silhouette_scores,"silhouette_scores_kmeans_rr.csv",sep=";")
write.table(kmeans_result$centers,"kmeans_result_centers_rr.csv",sep=";")
